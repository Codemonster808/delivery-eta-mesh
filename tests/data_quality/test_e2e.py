...
def test_full_pipeline_quality(worker_process):
    ...
    n_orders_generated = sum(
        ...
        if json.loads(line)["event_type"] == "order_placed"
    )
    ...
    accuracy = subprocess.run(
        [sys.executable, "scripts/accuracy.py", "--events", str(data_path)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        timeout=15,
    ...
    replay = subprocess.run(
        [sys.executable, "src/transformation/replay.py", "--in", str(data_path)],
        cwd=REPO_ROOT,
        capture_output=True,
        text=True,
        timeout=180,
    ...
    report.check(
        ...
        "every_published_order_gets_scored",
        measured=n_scored,
        threshold=n_orders_generated,
        detail=f"{n_scored}/{n_orders_generated} orders have an eta-current row",
    )
    report.check(
        Dimension.CORRECTNESS,
        "eta_mae_within_bound",
        measured=accuracy_result["mae_minutes"],
        threshold=12.0,
    ...
    salted_match = re.search(
        r"salted-then-reaggregated partition balance.*imbalance_ratio': ([\d.]+)", replay_out
    )
    ...
    report.check(
        Dimension.VALIDITY,
        "salting_reduces_partition_imbalance",
        measured=(salted_ratio if salted_ratio is not None else 999),
        threshold=(naive_ratio if naive_ratio is not None else 0),
        higher_is_better=False,
        detail=f"naive imbalance={naive_ratio}, salted imbalance={salted_ratio}",
    )
    report.check(
        Dimension.TIMELINESS,
        "worker_scores_orders_promptly",
        measured=round(worker_seconds, 1),
        threshold=20.0,
        higher_is_better=False,
        detail=f"{n_orders_generated} orders scored",
    )
    report.check(
        Dimension.TIMELINESS,
        "replay_job_under_sla",
        measured=round(replay_seconds, 1),
        threshold=180.0,
        higher_is_better=False,
        detail="PySpark watermark + salting job wall time",
    )
