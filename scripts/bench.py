...
def bench_worker_latency(n: int = 30) -> dict:
    ...
    for oid in order_ids:
        sqs.send_message(
            QueueUrl=url, MessageBody=json.dumps({"order_id": oid, "distance_km": 4.0})
        )
...
def main() -> None:
    ...
    print(
        "benchmarking worker throughput (requires the worker running: "
        "cd src/worker && mvn spring-boot:run)..."
    )
