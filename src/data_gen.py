...
scenario src/replay.py's Spark job has to handle without falling over on
one overloaded partition) and a fraction of GPS pings deliberately
arriving 5-15 minutes "late" (out of event-time order).
"""
import argparse
import json
import uuid
...
from common.synth import seeded_rng, skewed_choice  # noqa: E402
    ...
    with out_path.open("w") as f:
        for hour in range(args.hours):
            for _ in range(args.orders_per_hour):
                restaurant_id = skewed_choice(rng, restaurant_ids, hot_fraction=0.05, hot_weight=0.6)
                ...
                f.write(json.dumps({
                    ...
                    "ts": (gps_event_ts + arrival_delay).isoformat(),  # pipeline "sees" it at ts
                    "is_late": is_late,
                }) + "\n")
                n_events += 1
                n_late += int(is_late)

    top5pct = sorted(restaurant_order_counts.values(), reverse=True)[: max(1, args.restaurants // 20)]
