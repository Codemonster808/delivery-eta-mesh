...
LAMBDA_AVG_DURATION_MS = 50  # measured worker per-message processing time is a few ms; padded for cold-start variance


def fargate_cost_per_month(events_per_month: int) -> dict:
    hours_per_month = 24 * 30  # always-on, regardless of event volume
    cost = hours_per_month * (FARGATE_TASK_VCPU * FARGATE_VCPU_HOUR + FARGATE_TASK_GB * FARGATE_GB_HOUR)
    return {
        "monthly_cost_usd": round(cost, 2),
        "cost_per_million_events_usd": round(cost / (events_per_month / 1_000_000), 2) if events_per_month else None,
    }


def lambda_cost_per_month(events_per_month: int) -> dict:
    request_cost = events_per_month * LAMBDA_PER_REQUEST
    duration_cost = events_per_month * (LAMBDA_AVG_DURATION_MS / 1000) * LAMBDA_MEMORY_GB * LAMBDA_GB_SECOND
    total = request_cost + duration_cost
    return {
        "monthly_cost_usd": round(total, 2),
        "cost_per_million_events_usd": round(total / (events_per_month / 1_000_000), 2) if events_per_month else None,
    }


def crossover_events_per_month() -> int:
    """The event volume at which Fargate's fixed cost becomes cheaper than Lambda's per-event cost."""
    fargate_fixed = 24 * 30 * (FARGATE_TASK_VCPU * FARGATE_VCPU_HOUR + FARGATE_TASK_GB * FARGATE_GB_HOUR)
    lambda_per_event = LAMBDA_PER_REQUEST + (LAMBDA_AVG_DURATION_MS / 1000) * LAMBDA_MEMORY_GB * LAMBDA_GB_SECOND
...
def main() -> None:
    ...
    lines = [
        ...
        f"| $/million events | ${fargate['cost_per_million_events_usd']} | ${lam['cost_per_million_events_usd']} |",
        "",
        ...
        f"- Lambda: {LAMBDA_MEMORY_GB} GB memory, {LAMBDA_AVG_DURATION_MS}ms avg duration/invocation",
        "- Pricing: AWS us-east-1, on-demand, as published at https://aws.amazon.com/fargate/pricing/ "
    ...
    print(json.dumps({"fargate": fargate, "lambda": lam, "crossover_events_per_month": crossover}, indent=2))
    print(f"\nwrote {out_path}")
